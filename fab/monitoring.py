...
def check_load_average(limit):
    ...
    if (float(loads[0]) > float(limit)):
        _warning('Load average is %s which is above the threshold of %s.' % (str(loads[0]), str(limit)))
    else:
...
def check_disk_space(filesystem, limit):
    ...
    if (float(usage) > float(limit)):
        _warning('Disk usage of %s is at %s percent which is above the threshold of %s percent.' % (filesystem, str(usage), str(limit)))
    else:
...
def check_swap_usage(limit):
    ...
    if (usage > float(limit)):
        _warning('Swap usage is a %s percent which is above the threshold of %s percent.' % (str(usage), str(limit)))
    else:
...
def check_io_wait_time(limit):
    ...
    if (float(iowait) > float(limit)):
        _warning('IO wait times are at %s percent which is above the threshold of %s percent.' % (str(iowait), str(limit)))
    else:
...
def check_mysql(slow_query_limit, memory_usage, innodb_memory_usage, threads):
    with settings(warn_only=True):
        ...
        if report.failed:
            _failure('mysql server does not appear to be running: %s' % report)
...
def check_ldap():
    try:
        ...
        _failure('Cannot connect to LDAP on localhost.')
...
def check_pound_via_socket(port):
    ...
    try:
        ...
        _failure('Cannot connect to Pound on %s %s.' % ('localhost', str(port)))
...
def check_memcached(port):
    ...
    try:
        ...
        _failure('Cannot connect to Memcached on %s %s.' % ('localhost', str(port)))
...
def _test_url(service, url):
    ...
    if (status >=  400):
        _warning('%s returned an error code of %s.' % (service, status))
    else:
